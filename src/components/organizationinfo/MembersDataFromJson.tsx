...
const MembersDataFromJson = (props : membersDataFromJsonProps) => {
  const currentOrganizationMembersWithStatus = props.currentOrganizationMembers.filter((member: String[]) =>
  member[1] !== ""
  )
  ...
  const currentOrganizationMembersWithoutStatus = props.currentOrganizationMembers.filter((member: String[]) =>
  member[1] === ""
  )
      ...
      <div  className=' grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2  gap-4 rounded-lg mb-6'>
        ...
        return <span  className=' bg-gradient-to-b   from-slate-900 to-slate-800  rounded-sm' key = {"country-" + index}>
          ...
          {populationIcon}
            ...
            marginLeft: "auto",
            marginRight: "auto" }}
            className = "w-28 opacity-80 " src={element.flag_base64!}  />
        </span>
        })}
      </div>
      
        {
          Object.keys(currentOrganizationCountriesInfoWithStatuses).map((key, index) => {  

            
            return (
              <div className=' mb-6'>
              <h1 className=' text-2xl m-5 text-slate-200 uppercase'>{key}(s)</h1>
              <div className="grid grid-cols-4 content-center gap-4">
              {currentOrganizationCountriesInfoWithStatuses[key].map((element : any, index : number) => {
              return <span className=' bg-gradient-to-b   from-slate-900 to-slate-800  rounded-sm' key = {"country-" + index}>
                <h1 className='text-lg text-blue-800'>{element.country}</h1>
                <p data-tooltip-target="tooltip-continent">{element.continent}</p>
                <div id="tooltip-continent" role="tooltip" className="inline-block absolute invisible z-10 py-2 px-3 text-sm font-medium text-white bg-gray-900 rounded-lg shadow-sm opacity-0 transition-opacity duration-300 tooltip dark:bg-gray-700">
