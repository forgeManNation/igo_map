...
const SearchBar = (props: searchBarProps) => {
  ...
  return !sideBarLarge ? (
    <div
      className="bg-gray-800 text-white h-screen sticky top-0 overflow-hidden"
      style={{
    ...
    >
      {/* Searchbar and Title */}
      <div>
        <div className=" absolute  top-2 right-2" onClick={sideBarShow}>
          {arrowCircleLeft}{" "}
        </div>
        <h1
          className=" text-4xl mb-4 mt-5 cursor-pointer"
      ...
      <div className="overflow-y-auto  overflow-hidden">
        {Object.keys(organisations)
          .filter((org) => org.match(new RegExp(searchValue, "i")))
    ...
    <div className="bg-gray-800 text-white h-screen sticky top-0 overflow-hidden flex flex-col">
      <div className=" m-3" onClick={sideBarShow}>
        {arrowCircleRight}{" "}
      </div>
    </div>
  );
